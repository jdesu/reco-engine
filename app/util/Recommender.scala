...
object Recommender {
  ...
  def findOneAmazonCustomerForEmailAddress(EmailAddress: String): Future[Option[String]] = {
    val cursor = ReactiveDB.getAmazonCustomersForEmailAddress(EmailAddress)
    val x = for {
      a <- cursor.headOption
    } yield {
      Logger.debug(s"find One Amazon Customer For $EmailAddress: $a")
      a.map(_.customerId)
    }
    x
  }

  def findForCustomer(customerId: String): Future[Set[String]] = {
    val host = AppConfig.BRS.host
    val port = AppConfig.BRS.port
    val url = s"http://$host:$port/recommendations/foruser/$customerId"
    for (response <- WS.url(url).get()) yield {
      response.json.\("results").asInstanceOf[JsObject].keys.toSet.seq
    }
  }

  def findProductForCustomer(customerId: String): Future[JsValue] = {
    val host = AppConfig.PRS.host
    val port = AppConfig.PRS.port
    val url = s"http://$host:$port/recommendations/forproduct/$customerId"
    for (response <- WS.url(url).get()) yield {
      response.json
    }
  }
